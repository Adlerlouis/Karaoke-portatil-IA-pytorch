...
"""
. Embeddings sinusoidales

Estas funciones generan codificaciones posicionales sinusoidales, que son utiles para incorporar información de posición en datos secuenciales:

    create_sin_embedding:
    Genera un embedding sinusoidal de una dimension para una secuencia de longitud length y dimensión dim, donde cada dimension usa cosenos y senos con diferentes periodos maximos.

    create_2d_sin_embedding:
    Genera embeddings sinusoidales en 2D para datos con dimensiones height (alto) y width (ancho), usando divisiones de la dimensiOn total del modelo d_model.

    create_sin_embedding_cape:
    Variante avanzada que permite augmentación (desplazamiento global, local, escalado) en los embeddings sinusoidales para entrenamiento robusto. Esta versión puede normalizar los datos y añadir ruido aleatorio durante el entrenamiento.

2. Mascaras causales

Se usan para controlar qué elementos de una secuencia pueden ser atendidos durante el cálculo de atención en Transformers:

    get_causal_mask:
    Crea una máscara causal simple de longitud length, donde los elementos futuros son enmascarados (prohibidos) en cada posición.

3. Máscaras personalizadas

Funciones mas complejas para construir mascaras de atencion avanzadas:

    get_elementary_mask:
    Genera mascaras elementales con diferentes configuraciones:
        "diag": Mascara en ventana diagonal para atención local.
        "jmask": Mascara tipo triángulo jerárquico.
        "random": Mascara aleatoria basada en una probabilidad de sparsidad.
        "global": Mascara global que prioriza ciertas partes de la secuencia.

    get_mask:
    Combina varias mascaras elementales (por ejemplo, "diag_jmask_random") y las convierte en un tensor disperso (SparseCSRTensor), optimizado para eficiencia.
"""
...
def get_elementary_mask(
    ...
    """
Cuando la entrada del decodificador tiene una longitud T1 y la salida T2
La matriz de máscara tiene forma (T2, T1)
"""
...
def get_mask(
   ...
   """
Devuelve una máscara SparseCSRTensor que es una combinación de máscaras elementales
mask_type puede ser una combinación de múltiples máscaras: por ejemplo, "diag_jmask_random"
"""
...
#clasificacion de el encoder para el seguimiento del enconder 
class MyTransformerEncoderLayer(nn.TransformerEncoderLayer):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
       """
si batch_first = False, la forma de origen es (T, B, C)
no se contempla el caso en el que batch_first = True
"""
        device = src.device
        x = src
        T, B, C = x.shape
...
class CrossTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, q, k, mask=None):
        """
        Args:
            q: cantidad de tensores  (T, B, C)
            k: tensor of shape (S, B, C)
            masara:cantidad de tensores  (T, S)
...
La funcion de inicializacion define los parámetros, como la cantidad de capas, cabezas de atención, tipo de normalización, tipo de codificación posicional, y si las capas deben usar atención dispersa (sparse attention).

Parámetros importantes:

    dim: Dimension de la representación interna del transformer.
    num_heads: Numero de cabezas de atención.
    num_layers: Cantidad de capas alternadas entre atención clasica y cruzada.
    cross_first: Si True, las capas cruzadas son las primeras en el orden alternado.
    emb: Tipo de codificacion posicional. Puede ser sinusoidal (sin), codificacion posicional adaptable (cape), o escalada (scaled).
    sparse_self_attn y sparse_cross_attn: Si se debe usar atención dispersa en capas clasicas y cruzadas respectivamente.

2. Creacion de Capas

El modelo alterna entre capas de atencion clasicas y cruzadas:

    Capas clasicas: Procesan solo las entradas individuales (self.layers y self.layers_t).
    Capas cruzadas: Permiten interacciones entre dos entradas (x y xt).

Esto se logra verificando idx % 2 == self.classic_parity dentro de un bucle que define num_layers capas.

Cada capa se inicializa utilizando una configuración común (kwargs_common) que se ajusta segun el tipo de capa.
3. Normalizacion

Se emplean diversas estrategias de normalizacion:

    LayerNorm o MyGroupNorm, dependiendo de los argumentos norm_in o norm_in_group.
    En caso contrario, se usa Identity, que no realiza transformación alguna.

Esto asegura flexibilidad según las necesidades del modelo.
4. Codificación Posicional

El modelo usa codificaciones posicionales para incorporar información sobre la posicion relativa de las entradas:

    Sinusoidal (sin): Usa senos y cosenos para las posiciones.
    Cape: Añade normalización y modificaciones para mejorar el aprendizaje de posiciones.
    Scaled: Escala los valores posicionales.

Estas codificaciones se generan con funciones como create_2d_sin_embedding y _get_pos_embedding.
5. forward()

El metodo forward realiza el procesamiento de las entradas:

    Reorganizacion: Ajusta la forma de las entradas (x, xt) para que sean compatibles con las capas.
    Añade Embeddings Posicionales: Integra la información posicional en las entradas.
    Paso por las Capas: Alterna entre capas clásicas y cruzadas.
    Reconstruccion: Reorganiza las salidas a sus formas originales.

6. Atencion

El codigo define multiples métodos relacionados con la atencion, incluidas las siguientes estrategias:

    Atención clasica (scaled dot product): Calcula la atencion utilizando mascaras y matrices escaladas.
    Atencion dispersa (dynamic sparse attention): Usa técnicas optimizadas para reducir el costo computacional, seleccionando ubicaciones relevantes en las entradas.

Estas funciones aseguran que el modelo pueda manejar datos grandes y jerarquicos.
"""
